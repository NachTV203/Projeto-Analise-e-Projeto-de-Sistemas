# UDF - CENTRO UNIVERSITÁRIO DO DISTRITO FEDERAL

## NOME DO CURSO  

**DISCIPLINA – ANÁLISE E PROJETO DE SISTEMAS**  

---

# SISTEMA XYZ  
**DOCUMENTAÇÃO DE NEGÓCIOS, REQUISITOS E MODELAGEM UML**

**Alunos:**  
- Nome Completo - RGM  
- Nome Completo - RGM  
- Nome Completo - RGM  
- Rafael Junio Azevedo Souza - RGM 38595435 

**Professor:** Gabriel de Oliveira Alves  

Brasília – 2025.2  

---

## Sumário
1. Introdução  
2. Justificativa  
3. Objetivos  
   - 3.1 Objetivo Geral  
   - 3.2 Objetivos Específicos  
4. Descrição do Sistema Proposto  
5. Requisitos  
   - 5.1 Requisitos Funcionais  
   - 5.2 Requisitos Não Funcionais  
   - 5.3 Regras de Negócio  
6. Diagramas UML  
   - 6.1 Diagrama de Casos de Uso  
   - 6.2 Diagrama de Classe  
   - 6.3 Diagrama de Atividades  
   - 6.4 Diagrama de Sequência  
   - 6.5 Diagrama de Estado  
   - 6.6 Diagrama de Componentes  
   - 6.7 Diagrama de Objetos  
7. Conclusão  
8. Referências  

---

## 1. Introdução
*Explique o contexto do trabalho, apresente o tema do sistema proposto e mostre a relevância da documentação de requisitos e da modelagem UML no desenvolvimento de software.*  

---

## 2. Justificativa
*Por que este sistema é importante? Quais problemas resolve e qual o impacto esperado?*  

---

## 3. Objetivos  

### 3.1 Objetivo Geral  
*Exemplo: Desenvolver a documentação de requisitos e modelagem UML de um sistema de gestão de bicicletas.*  

### 3.2 Objetivos Específicos  
- Identificar e descrever os requisitos funcionais e não funcionais.  
- Elaborar diagramas UML.  
- Produzir documentação de negócios e requisitos.  
- Aplicar metodologias de elicitação e análise de requisitos.  

---

## 4. Descrição do Sistema Proposto
*Explique qual sistema será desenvolvido, público-alvo, principais funcionalidades e tecnologias.*  

---

## 5. Requisitos  

### 5.1 Requisitos Funcionais  
Exemplo:  
- **RF01 – Cadastro de Cliente**: O sistema deve permitir o cadastro de clientes, incluindo nome completo, CPF, endereço, telefone e e-mail.  

### 5.2 Requisitos Não Funcionais  
Exemplo:  
- **RNF01 – Desempenho**: O sistema deve responder às solicitações em até 2 segundos.  

### 5.3 Regras de Negócio  
Exemplo:  
- O cliente só pode ser cadastrado se informar CPF válido e não estiver previamente registrado no sistema.  

---

## 6. Diagramas UML  

### 6.1 Casos de Uso  
Resumo e descrição dos casos de uso.  
Inserir diagramas aqui.  

### 6.2 Diagrama de Classe  
Resumo e imagem do diagrama.  

### 6.3 Diagrama de Atividades  
Resumo e imagem do diagrama.  

### 6.4 Diagrama de Sequência  
Resumo e imagem do diagrama.  

### 6.5 Diagrama de Estado  
Resumo e imagem do diagrama.  

### 6.6 Diagrama de Componentes  
Resumo e imagem do diagrama.  

### 6.7 Diagrama de Objetos  
Resumo e imagem do diagrama.  

---

## 7. Conclusão
*Resumo sobre a experiência: aprendizados, dificuldades e contribuições da modelagem.*  

---

## 8. Referências
*Inserir as referências em formato ABNT.*  

---

## Ficha de Autoavaliação do Grupo  

| Integrantes       | Discussões | Produção escrita | Diagramas UML | Organização | Apresentação |
|-------------------|------------|------------------|---------------|-------------|--------------|
| Nome Completo 01  |            |                  |               |             |              |
| Nome Completo 02  |            |                  |               |             |              |
| Nome Completo 03  |            |                  |               |             |              |
| Nome Completo 04  |            |                  |               |             |              |

---
